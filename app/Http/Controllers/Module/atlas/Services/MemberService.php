...
namespace App\Http\Controllers\Module\atlas\Services;
...
class MemberService {
    ...
    static function sendSmsBindCode($params = [], $login_user) {
        if (empty($params['phone'])) return return_api_format(['status' => 0, 'msg' => '请输入绑定的手机号！']);
        if (!is_mobile($params['phone'])) return return_api_format(['status' => 0, 'msg' => '请输入正确的手机号！']);
        if ($login_user['phone_active'] == 1 && $params['phone'] == $login_user['phone']) return return_api_format(['status' => 0, 'msg' => '您已绑定该手机号！']);
    ...
    static function checkSmsBindCode($params = [], $login_unique = Member::LOGIN_UNIQUE) {
        ...
        if (empty($verify_code = MembersVerifyLogs::getLastVerifyCode($params['uid'], 1, 1))) return return_api_format(['status' => 0, 'msg' => '验证码已失效，请重新发送！']);
        if ($verify_code->verify_receive != $params['phone']) return return_api_format(['status' => 0, 'msg' => '该手机号与验证码不匹配！']);
        if (empty($params['phone_code'])) return return_api_format(['status' => 0, 'msg' => '请重新发送验证码！']);
        if ($verify_code->verify_code != $params['phone_code']) return return_api_format(['status' => 0, 'msg' => '该验证码不匹配！']);
        if (empty($user = Member::lockForUpdate()->find($params['uid']))) return return_api_format(['status' => 0, 'msg' => '账户已丢失，请重新登陆！']);
        ...
        try {
            ...
            DB::rollback();
            return return_api_format(['status' => 0, 'msg' => '手机号绑定失败！']);
        }
    }
    ...
    static function sendSmsUntyingCode($params = [], $login_user = []) {
        if (empty($params['phone'])) return return_api_format(['status' => 0, 'msg' => '请输入解绑的手机号！']);
        if (!is_mobile($params['phone'])) return return_api_format(['status' => 0, 'msg' => '请输入正确的手机号！']);
        if ($params['phone'] != $login_user['phone']) return return_api_format(['status' => 0, 'msg' => '您尚未绑定该手机号，无需解绑！']);
    ...
    static function checkSmsUntyingCode($params = [], $login_unique = Member::LOGIN_UNIQUE) {
        ...
        if (empty($verify_code = MembersVerifyLogs::getLastVerifyCode($params['uid'], 1, 1, 0))) return return_api_format(['status' => 0, 'msg' => '验证码已失效，请重新发送！']);
        if ($verify_code->verify_receive != $params['phone']) return return_api_format(['status' => 0, 'msg' => '该手机号与验证码不匹配！']);
        if (empty($params['phone_code'])) return return_api_format(['status' => 0, 'msg' => '请重新发送验证码！']);
        if ($verify_code->verify_code != $params['phone_code']) return return_api_format(['status' => 0, 'msg' => '该验证码不匹配！']);
        if (empty($user = Member::lockForUpdate()->find($params['uid']))) return return_api_format(['status' => 0, 'msg' => '账户已丢失，请重新登陆！']);
        ...
        try {
            ...
            DB::rollback();
            return return_api_format(['status' => 0, 'msg' => '手机号解绑失败！']);
        }
    }
    ...
    static function sendEmailBindCode($params = []) {
        if (empty($params['email'])) return return_api_format(['status' => 0, 'msg' => '请输入绑定的邮箱账户！']);
    ...
    static function checkEmailBindCode($params = [], $login_unique = Member::LOGIN_UNIQUE) {
        ...
        if (empty($verify_code = MembersVerifyLogs::getLastVerifyCode($params['uid'], 0, 1))) return return_api_format(['status' => 0, 'msg' => '验证码已失效，请重新发送！']);
        if ($verify_code->verify_receive != $params['email']) return return_api_format(['status' => 0, 'msg' => '该邮箱与验证码不匹配！']);
        if (empty($params['email_code'])) return return_api_format(['status' => 0, 'msg' => '请重新发送验证码！']);
        if ($verify_code->verify_code != $params['email_code']) return return_api_format(['status' => 0, 'msg' => '该验证码不匹配！']);
        if (empty($user = Member::lockForUpdate()->find($params['uid']))) return return_api_format(['status' => 0, 'msg' => '账户已丢失，请重新登陆！']);
        ...
        try {
            ...
            DB::rollback();
            return return_api_format(['status' => 0, 'msg' => '邮箱绑定失败！']);
        }
    }
    ...
    static function sendEmailUntyingCode($params = []) {
        if (empty($params['email'])) return return_api_format(['status' => 0, 'msg' => '请输入解绑的邮箱账户！']);
        $params['email_code'] = random_verification_code(6);//验证码
    ...
    static function checkEmailUntyingCode($params = [], $login_unique = Member::LOGIN_UNIQUE) {
        ...
        if (empty($verify_code = MembersVerifyLogs::getLastVerifyCode($params['uid'], 0, 1, 0))) return return_api_format(['status' => 0, 'msg' => '验证码已失效，请重新发送！']);
        if ($verify_code->verify_receive != $params['email']) return return_api_format(['status' => 0, 'msg' => '该邮箱与验证码不匹配！']);
        if (empty($params['email_code'])) return return_api_format(['status' => 0, 'msg' => '请重新发送验证码！']);
        if ($verify_code->verify_code != $params['email_code']) return return_api_format(['status' => 0, 'msg' => '该验证码不匹配！']);
        if (empty($user = Member::lockForUpdate()->find($params['uid']))) return return_api_format(['status' => 0, 'msg' => '账户已丢失，请重新登陆！']);
        ...
        try {
            ...
            DB::rollback();
            return return_api_format(['status' => 0, 'msg' => '邮箱解绑失败！']);
        }
    }
    ...
    static function saveUserInfo($request, $user, $login_unique = Member::LOGIN_UNIQUE) {
        ...
        if (!$res) return return_api_format(['status' => 0, 'msg' => '更新失败！']);
    ...
    public static function getCode($type, $params) {
        ...
        if ($type['updateType'] == 1) {
            if ($user) {
                $type['msg'] = '登录账号';
                $type['getType'] = 2;
                $type['template'] = 'sms_bind_template';//暂时用户绑定的
            } else {
                $type['msg'] = '注册账户';
                $type['getType'] = 1;
        ...
        } elseif (in_array($type['getType'], [7, 9])) {
        } else {
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public static function getRegisterCode($params, $template = '') {
        if (__E("website_open_reg") != 1) return return_api_format(['status' => 0, 'msg' => '系统关闭注册功能！']);
        $validator = Validator::make($params, [
            'phone' => 'required',
        ], [
            'phone.required' => '手机号为必填项！',
        ...
        if (!empty($user = Member::where('type', 'member')
            ...
            ->first())) return return_api_format(['status' => 0, 'msg' => '手机号已存在']);

        if (!empty(MembersVerifyLogs::getLastVerifyCode(0, 1, 1, 1, $params['phone']))) return return_api_format(['status' => 200, 'msg' => '您已发送验证码！']);
        $params['phone_code'] = random_verification_code(6);//验证码
        if ($template == 'sms_bind_template') {
            $template = 'sms_bind_template';//绑定手机号
        } else {
            $template = 'sms_register_template';//注册用户
        }
        $template_content = TemplateMessage::getTemplateMessage($template, $params['phone_code']);
        if ($template_content['status'] != 1) return return_api_format(['status' => 0, 'msg' => $template_content['msg']]);
        $send_content = $template_content['data'];
    ...
    public static function getForgetCode($params) {
        ...
        if (empty($user = Member::where('type', 'member')
            ->where('phone', $params['phone'])
            ->first())) return return_api_format(['status' => 0, 'msg' => '手机号不存在']);
        if (intval($user->status) != 1) return return_api_format(['status' => 0, 'msg' => '该账户已被禁用']);


        if (!empty($re = MembersVerifyLogs::getLastVerifyCode($user->uid, 1, 1, 1, $params['phone']))) {
            return return_api_format(['status' => 200, 'msg' => '您已发送验证码！']);
        }
    ...
    public static function forget($params) {
        ...
        try {
            //1.更新会员的资料
            $user->update_at = date('Y-m-d H:i:s');
            $user->password = Member::getPassword($params['password']);
            $user->save();
