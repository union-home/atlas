...
<div class="page-container">
                    ...
                    <div class="col-sm-12">
                        <div class="table-responsive panel panel-default">
                            <div class="panel-heading">
                                <a class="label bg-info pull-right m-t-xs {{atlasPermissions('group/groupAdd')}}"
                                   href="javascript:void(0)" id="addPage">
                                    添加
                                </a>
                                列表
                            </div>
                                ...
                                @forelse($data as $d)
                                    <tr>
                                       ...
                                       {{-- <td>
                                        ...
                                        <td>
                                            <a href="{{atlasModuleAdminJump($moduleName,'group/groupUsers?id='.$d['id'])}}"
                                               class="{{atlasPermissions('group/groupUsers')}}">
                                                <button type="button" class="h-button-edit btn btn-info btn-xs">
                                                    组成员
                                                </button>
                                            </a>
                                            <a href="{{atlasModuleAdminJump($moduleName,'group/assignPermissions?id='.$d['id'])}}"
                                               class="{{atlasPermissions('group/assignPermissions')}}">
                                                <button type="button" class="h-button-edit btn btn-info btn-xs">
                                                    分配权限
                                                </button>
                                            </a>
                                            <a href="javascript:void(0)"
                                               class="{{atlasPermissions('group/groupAdd')}}"
                                            ...
                                            <button type="button"
                                                    class="btn btn-danger btn-xs {{atlasPermissions('group/delete')}}"
                                                    onclick="_confirm('{{atlasModuleAdminJump($moduleName,'group/groupDelete')}}',{
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-11" style="margin-bottom: 15px;">
                        <select name="type" class="form-control" id="type">
                            @foreach(atlasUserType() as $u)
                                <option value="{{$u['tig']}}">{{$u['name']}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
    ...
    <script>
        ...
        $('#add').click(function () {
            ...
            var type = $('#type').val();
            if (!group_name) return layer.msg('名称不能为空', {icon: 2});
            if (!type) return layer.msg('用户类型不能为空', {icon: 2});
